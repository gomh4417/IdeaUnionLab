...
export default function ItemList({ items = [], activatedIdx = null, onDeleteItem, originalItems = [] }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginTop:"8px" }}>
      {items.map((item, idx) => (
        <Item
          key={item.id || idx} // ID가 있으면 ID를 key로 사용
          idx={idx}
          imageUrl={item.imageUrl}
          title={item.title}
          tags={item.tags}
          type={activatedIdx === idx ? 'activated' : 'default'}
          onDelete={handleDelete} // 래퍼 함수 전달
          itemData={item} // 전체 아이디어 데이터 전달
        />
      ))}
    </div>
  );
